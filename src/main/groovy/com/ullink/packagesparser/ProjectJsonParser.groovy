    ...
    Collection getDependencies(File file) {
        ...
        projectJson.dependencies.each { packageElement ->
            if (!isBuildDependency(packageElement))
                dependencies.add({
                    dependency(id: packageElement.key, version: getResolvedVersion(packageElement, projectLockJson))
                })
        }
        return dependencies
    }

    static boolean isBuildDependency(Map.Entry entry) {
        return entry.value instanceof Map && entry.value.type == 'build'
    }

    static String getVersion(Map.Entry entry) {
        return entry.value instanceof Map ? entry.value.version : entry.value
    }

    static Boolean isBracket(String character) {
        return character == '[' || character == '(' || character == ']' || character == ')'
    }

    static String getResolvedVersion(Map.Entry entry, Object projectLockJson) {
